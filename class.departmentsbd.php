...
#	System functions
	...
	class Departmentsbd{
		...
		static function listDepartments($oLimit = false){
			...
			$Query = "SELECT ".DEPARTMENTS_BD.".* FROM `".DEPARTMENTS_BD."`";
		...
		static function listDepartmentsByCustomer($CustomerId, $See){
			...
			$Database = new Database();
			if($See == 4){
				$Query = "SELECT ".DEPARTMENTS_BD."_bd.* FROM `".DEPARTMENTS_BD."_bd`  WHERE ".DEPARTMENTS_BD."_bd.`option1` = '1' ORDER BY ".DEPARTMENTS_BD."_bd.`departmentcode`";
				if($oLimit) $Query.= " LIMIT ".$oLimit;
			}
			else{
				$Query = "SELECT ".DEPARTMENTS_BD.".* FROM `".DEPARTMENTS_BD."`  WHERE ".DEPARTMENTS_BD.".`option1` = '1' ORDER BY ".DEPARTMENTS_BD.".`departmentcode`";
				if($oLimit) $Query.= " LIMIT ".$oLimit;
			}
			$Department = $Database->Select($Query, rows_object);
